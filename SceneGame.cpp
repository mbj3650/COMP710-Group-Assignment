	...
	, m_rotationSpeed(1.0f)
{
	...
	// game over start as false of course
	m_bGameOver = false;
	m_pGameOverSprite = 0;
}
...
bool SceneGame::Initialise(Renderer& renderer)
{
	...
	// load the game over image and centre it on screen
	// make sure defeated_gameover.png is inside the assets folder
	m_pGameOverSprite = renderer.CreateSprite("..\\assets\\defeated_gameover.png");
...
SceneGame::Process(float deltaTime, InputSystem& inputSystem)
{
	...
	// if game over, dont process anything else
	if (m_bGameOver)
	{
		return;
	}
	...
	else
	{
		...
		for (int i = (int)m_enemies.size() - 1; i >= 0; i--)
		{
			...
			if (m_enemies[i]->HasReachedEnd())
			{
				...
				// check if lives is zero, if yes trigger game over
				if (m_iLives <= 0)
				{
					m_bGameOver = true;
					std::cout << "game over!\n";
		...
		if (!m_bGameOver && m_iEnemiesToSpawn == 0 && m_enemies.empty())
		{
			m_iWave++;
...
SceneGame::Draw(Renderer& renderer)
{
	...
	// if game over, draw the game over image on top of everything
	if (m_bGameOver)
	{
